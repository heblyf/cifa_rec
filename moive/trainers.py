...
class Trainer:
    ...
    def cross_entropy(self, seq_out, pos_ids, neg_ids):
        ...
        loss = torch.mean(
            -torch.log(torch.sigmoid(pos_logits) + 1e-24) - torch.log(1 - torch.sigmoid(neg_logits) + 1e-24)
        )  # / torch.sum(istarget)
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                # input_ids 输入 item 序列
                # ratings 用户对每个 item 的评分
                # answer 预测 item
                # neg_answer 负采样 item
                _, input_ids, ratings, genres, users, users_gender, users_age, answer, neg_answer = batch
                sequence_output = self.model(input_ids, ratings, genres, users, users_gender, users_age)
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    batch = tuple(t.to(self.device) for t in batch)
                    user_ids, input_ids, ratings, genres, users, users_gender, users_age, answers, _, neg_answer = batch
                    recommend_output = self.model(input_ids, ratings, genres, users, users_gender, users_age)
                    # batch_size*hidden_size
                    recommend_output = recommend_output[:, -1, :]

                    # 为用户推荐 20 个 items
                    # user 在所有 item 上的评分，batch_size*max_item
                    rating_pred = self.predict_full(recommend_output)
                    rating_pred = rating_pred.cpu().data.numpy().copy()
                    batch_user_index = user_ids.cpu().numpy()
                    # 去除 user 实际关联的 item，置为零
                    ...
                    # 根据序号得到 topk 评分值（未按照从大到小进行排序）
                    arr_ind = rating_pred[np.arange(len(rating_pred))[:, None], ind]
                    # 获取 topk 评分值按照从大到小评分的序号
