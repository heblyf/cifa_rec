...
class FMLPRecDataset(Dataset):
    def __init__(self, args, seq_dic, test_neg_items=None, data_type="train"):
        ...
        if data_type == "train":
            ...
            for j in range(n_total):
                ...
                genres = genres_seq[j][-(self.max_len + 2) : -2]
                for i in range(len(genres)):
                    self.genres_seq.append(genres[: i + 1])
                for i in range(c):
                    self.user_id_seq.append(user_id_seq[j])
                    self.user_gender_seq.append(user_gender_seq[j])
            ...
            for i in range(n_total):
                ...
                self.genres_seq.append(genres_seq[i][:-1])
                self.user_id_seq.append(user_id_seq[i])
                self.user_gender_seq.append(user_gender_seq[i])
            ...
            self.user_gender_seq = user_gender_seq
    ...
    def __getitem__(self, index):
        ...
        genres = self.genres_seq[index][:-1]

        user_id = self.user_id_seq[index]
        user_gender = self.user_gender_seq[index]
        ...
        genres = [[0 for i in range(self.num_genres)] for i in range(pad_len)] + genres
        genres = genres[-self.max_len :]
        assert len(genres) == self.max_len
        ...
        if self.test_neg_items is not None:
            ...
            cur_tensors = (
                ...
                torch.tensor(user_gender, dtype=torch.long),
