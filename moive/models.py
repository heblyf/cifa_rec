...
class FMLPRecModel(nn.Module):
    def __init__(self, args):
        ...
        # genres embedding
        self.genres_embeddings = nn.Linear(args.num_genres, args.hidden_size)
        # user embedding
        self.user_embeddings = nn.Embedding(args.num_users, args.hidden_size)
        # user gender embedding
        self.user_gender_embeddings = nn.Embedding(args.num_user_gender, args.hidden_size)
        # user age embedding
        self.user_age_embeddings = nn.Embedding(args.num_user_age, args.hidden_size)
        # AdaNorm layer
    ...
    def embedding(self, sequence, ratings, genres, users, users_gender, users_age):
        ...
        # genres
        genres_embeddings = self.genres_embeddings(genres)

        batch_size = users.shape[0]
        users = users.unsqueeze(1).expand(batch_size, seq_length)
        users_gender = users_gender.unsqueeze(1).expand(batch_size, seq_length)
        users_age = users_age.unsqueeze(1).expand(batch_size, seq_length)

        # users
        user_embeddings = self.user_embeddings(users)
        # user gender
        user_gender_embeddings = self.user_gender_embeddings(users_gender)
        # user age
    ...
    # same as SASRec
    def forward(self, input_ids, ratings, genres, users, users_gender, users_age):
        ...
        sequence_emb = self.embedding(input_ids, ratings, genres, users, users_gender, users_age)
