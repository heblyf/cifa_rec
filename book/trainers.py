    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                # answer 预测 item
                # neg_answer 负采样 item
                _, input_ids, ratings, users, users_age, answer, neg_answer = batch
                sequence_output = self.model(input_ids, ratings, users, users_age)
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    batch = tuple(t.to(self.device) for t in batch)
                    user_ids, input_ids, ratings, users, users_age, answers, _, neg_answer = batch
                    recommend_output = self.model(input_ids, ratings, users, users_age)
