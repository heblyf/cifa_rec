...
sequential_data_list = ["Beauty", "Sports_and_Outdoors", "Toys_and_Games", "Yelp", "ml_1m", "book_recommend"]
...
def get_user_seqs_and_sample(data_file, sample_file):
    ...
    for line in lines:
        ...
        user_seq.append(items)
        item_set = item_set | set(items)
    max_item = max(item_set)  # item 的最大个数
    ...
    # user_seq 用户对应的 item 序列
    # XXX sample_seq 负采样得到的 item 序列
